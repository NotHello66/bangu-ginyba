...
public partial class Enemy : CharacterBody3D
{
    ...
    [Export] private PackedScene goldPickupScene;
    ...
    private void SpawnGoldPickup()
    {
        if (goldPickupScene == null) return;
        var pickup = goldPickupScene.Instantiate<GoldPickup>();
        GetTree().CurrentScene.AddChild(pickup);
        pickup.GlobalPosition = GlobalPosition + Vector3.Up * 0.5f;
        pickup.SetValue(GoldReward);
    }
    ...
    {
        currentVelocity.X = Mathf.MoveToward(currentVelocity.X, 0f, (float)delta * knockBackResist);
        currentVelocity.Z = Mathf.MoveToward(currentVelocity.Z, 0f, (float)delta * knockBackResist);
