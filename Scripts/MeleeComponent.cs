...
public partial class MeleeComponent : Node3D
{
	...
	[Export] public float[] stunDurations;
	...
	public void PerformAttack(int index)
	{
		...
		attackNode.Initialize(new Attack(GetDamage(index), GetKnockback(index), GlobalPosition, GetStunDuration(index)));
	}
	...
	public void PerformAttack()
	{
		for (int i = 0; i < attackScenes.Length; i++)
		{
			if (timers[i] >= GetCooldown(i))
			{
				...
				attackNode.GlobalPosition = GlobalPosition;
				attackNode.GlobalRotation = facingRotation;
				attackNode.Initialize(new Attack(GetDamage(i), GetKnockback(i), GlobalPosition, GetStunDuration(i)));
			}
		}
	}
	...
	private float GetDamage(int index)
	{
		...
		return damages[Mathf.Min(index, damages.Length - 1)] + BonusDamage;
	}
	...
	public float GetStunDuration(int index)
	{
		// jei per mazai cooldownu, naudoja paskutini
		if (stunDurations == null || stunDurations.Length == 0) return 0f;
		return stunDurations[Mathf.Min(index, stunDurations.Length - 1)];
	}
